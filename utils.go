...
const (
	LENOF_BIGINT                           = 32
	LENOF_HASH                             = 32
	LENOF_RIPEMD160ADDRESS                 = 20
	SIZEOF_RIPEMD160ADDRESS                = 1 + LENOF_RIPEMD160ADDRESS
	CRYPTOTYPEOF_ECDSA256_SHA256_RIPEMD160 = iota
)

var (
	ERR_SIZEOF_BYTES_INCORRECT = errors.New("Size of bytes is incorrect.")
)

var (
	MNEMONICS_ENGLISH = []string{
		...
		"donor", "door", "dose", "double", "dove", "draft", "dragon", "drama", "drastic", "draw", "dream", "dress",
		"drift", "drill", "drink", "drip", "drive", "drop", "drum", "dry", "duck", "dumb", "dune", "during", "dust",
		...
		"ensure", "enter", "entire", "entry", "envelope", "episode", "equal", "equip", "era", "erase", "erode",
		...
		"fade", "faint", "faith", "fall", "false", "fame", "family", "famous", "fan", "fancy", "fantasy", "farm",
		...
		"floor", "flower", "fluid", "flush", "fly", "foam", "focus", "fog", "foil", "fold", "follow", "food", "foot",
		"force", "forest", "forget", "fork", "fortune", "forum", "forward", "fossil", "foster", "found", "fox",
		...
		"hungry", "hunt", "hurdle", "hurry", "hurt", "husband", "hybrid", "ice", "icon", "idea", "identify",
		...
		"joy", "judge", "juice", "jump", "jungle", "junior", "junk", "just", "kangaroo", "keen", "keep", "ketchup",
		"key", "kick", "kid", "kidney", "kind", "kingdom", "kiss", "kit", "kitchen", "kite", "kitten", "kiwi", "knee",
		...
		"light", "like", "limb", "limit", "link", "lion", "liquid", "list", "little", "live", "lizard", "load",
		...
		"person", "pet", "phone", "photo", "phrase", "physical", "piano", "picnic", "picture", "piece", "pig",
		...
		"shrug", "shuffle", "shy", "sibling", "sick", "side", "siege", "sight", "sign", "silent", "silk", "silly",
		...
		"tackle", "tag", "tail", "talent", "talk", "tank", "tape", "target", "task", "taste", "tattoo", "taxi", "teach",
		...
		"trap", "trash", "travel", "tray", "treat", "tree", "trend", "trial", "tribe", "trick", "trigger", "trim",
		"trip", "trophy", "trouble", "truck", "true", "truly", "trumpet", "trust", "truth", "try", "tube", "tuition",
	...
	LENOF_MNEMONICS_ENGLISH = len(MNEMONICS_ENGLISH)
)

func IndexOf(strs []string, str string) (index int) {
...
func Bytes2Mnemonics(bytes []byte) (mnemonics []string) {
	base := big.NewInt(int64(len(MNEMONICS_ENGLISH)))
	...
	for result.Cmp(zero) != 0 {
		...
		tmp := big.NewInt(0)
		tmp.DivMod(result, base, mod)
		mnemonics = append(mnemonics, MNEMONICS_ENGLISH[mod.Int64()])
		//println("/", base.String(), "=", tmp.String(), "mod", mod.String(), "word", MNEMONICS_ENGLISH[mod.Int64()])
		result = tmp
	}
...
func Mnemonics2Bytes(mnemonics []string) []byte {
	base := big.NewInt(int64(len(MNEMONICS_ENGLISH)))
	poweredbase := big.NewInt(1)
	result := big.NewInt(0)
	mul := big.NewInt(0)

	for n := len(mnemonics) - 1; n >= 0; n-- {
		mnemonic := mnemonics[n]
		i := IndexOf(MNEMONICS_ENGLISH, mnemonic)
		if i == -1 {
			return nil
		}

		mul.Mul(result, poweredbase)
		//println("word", mnemonic, ":", result.String(), "*", poweredbase.String(), "=", mul.String(), "+", i, "=")
		result.Add(big.NewInt(int64(i)), mul)
		//println(result.String())
		if poweredbase.Cmp(big.NewInt(1)) == 0 {
			poweredbase = base
		}
	}
...
func (this *Signable) Abstract() []byte {
	return Sha256Encrypt(this.bytes)
}

func NewSignable(str string) (result *Signable, err error) {
	bytes, err := Base64Decode(str)
	if err != nil {
		return nil, err
	}

	return &Signable{bytes: bytes}, err
}

type ECDSAUtils struct{}

func (this *ECDSAUtils) AddressFromBytes(bytes []byte) (address IAddress) {
	address = &Ripemd160Address{}
	address.SetData(bytes)
	return
}

func (this *ECDSAUtils) PrivateKeyFromBytes(bytes []byte) (privateKey IPrivateKey) {
	privateKey = &PrivateKey{}
	privateKey.SetData(bytes)
	return
}

func (this *ECDSAUtils) NewPrivateKey() IPrivateKey {
	return &PrivateKey{}
}

func (this *ECDSAUtils) PublicKeyFromBytes(bytes []byte) (publicKey IPublicKey) {
	publicKey = &PublicKey{}
	publicKey.SetData(bytes)
	return
}

func (this *ECDSAUtils) SignatureFromBytes(bytes []byte) (signature ISignature) {
	signature = &Signature{}
	signature.SetData(bytes)
	return
}

func (this *ECDSAUtils) PublicKeyFromPrivateString(pristr string) (pubstr string, err error) {
	pribytes, err := Base64Decode(pristr)
	if err != nil {
		return "", err
	}

	privateKey := (&PrivateKey{})
	err = privateKey.SetData(pribytes)
	if err != nil {
		return "", err
	}

	publicKey := privateKey.PublicKey()
	return publicKey.String(), err
}

func (this *ECDSAUtils) SignDataByPrivateString(pristr string, datastr string) (signstr string, err error) {
	pribytes, err := Base64Decode(pristr)
	if err != nil {
		return
	}
	...
	signable, err := NewSignable(datastr)
	if err != nil {
		return
	}

	signature := privateKey.Sign(signable)
	return signature.String(), err
}

func (this *ECDSAUtils) VerifySignatureAndDataByPublicString(signstr string, pubstr string, datastr string) (result int, err error) {
	signature := &Signature{}
	signature.SetString(signstr)

	publicKey := &PublicKey{}
	publicKey.SetString(pubstr)
	...
	if publicKey.Verify(signable, signature) {
		return 1, err
	}
